  ...
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-6 pt-28 pb-20 grid lg:grid-cols-2 gap-16 items-center">
        {/* Text */}
        <div>
          <div className="section-label mb-6">{t.hero.label}</div>

          <h1 className="font-display text-5xl lg:text-7xl font-bold leading-[1.05] mb-6">
            <span className="text-ivory-100">{titleParts[0]}</span>
            <br />
            <span className="gold-text">{titleParts[1]}</span>
            <br />
            <span className="text-ivory-100">{titleParts[2]}</span>
            <span className="text-ivory-300"> {titleParts[3]}</span>
          </h1>
          ...
          <div className="flex flex-wrap gap-2 mb-10">
            {TECH_PILLS.map(({ icon: Icon }, index) => {
              const labels = [t.hero.pills.web, t.hero.pills.ai, t.hero.pills.data, t.hero.pills.cloud];
              return (
                <div
                  key={labels[index]}
          ...
          <div className="mt-12 flex items-center gap-6">
            ...
            <div>
              <div className="text-ivory-200 font-bold text-sm">{t.hero.students}</div>
              <div className="text-ivory-500 text-xs">{t.hero.enrolled}</div>
            </div>
          </div>
        </div>
        ...
        <div className="hidden lg:flex flex-col items-center justify-center relative">
          ...
          <div className="text-center mt-4">
            <div className="text-gold-500 text-xs tracking-[0.3em] uppercase font-semibold">{t.hero.visualLabel}</div>
            <div className="text-ivory-500 text-xs mt-1">{t.hero.visualSub}</div>
          </div>

          {/* Floating stat cards */}
          <div className="glass-card rounded-lg p-4 absolute top-0 right-0 text-center min-w-[110px]">
            <div className="stat-num text-2xl font-bold">{t.hero.stat1.value}</div>
            <div className="text-ivory-500 text-xs mt-1">{t.hero.stat1.label}</div>
          </div>
          <div className="glass-card rounded-lg p-4 absolute bottom-8 left-0 text-center min-w-[110px]">
            <div className="stat-num text-2xl font-bold">{t.hero.stat2.value}</div>
            <div className="text-ivory-500 text-xs mt-1">{t.hero.stat2.label}</div>
          </div>
        </div>
      </div>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 animate-bounce">
        <div className="text-ivory-600 text-xs tracking-widest uppercase">{t.hero.scroll}</div>
